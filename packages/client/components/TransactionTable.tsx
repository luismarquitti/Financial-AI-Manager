          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedTransactions.length > 0 ? paginatedTransactions.map((transaction) => (
              <tr key={transaction.id} className={`hover:bg-gray-50 ${selectedIds.has(transaction.id) ? 'bg-indigo-50' : ''}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{transaction.category ?? 'N/A'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{transaction.account ?? 'N/A'}</td>
