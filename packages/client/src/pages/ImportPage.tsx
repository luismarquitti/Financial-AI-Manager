  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">{t('importPage.title')}</h2>
            <p className="text-text-secondary mt-1">{t('importPage.subtitle')}</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            >
              {isLoading 
                ? <><svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> {t('importPage.saving')}</>
                : <><SaveIcon /> {t('importPage.saveButton', { count: selectedCount })}</>
              }
            </button>
          </div>
      </div>
      ...
      <div className="bg-card shadow-lg rounded-xl p-6">
        <div className="mb-4 flex justify-between items-center">
            <h3 className="text-xl font-bold">{t('importPage.stagedTitle', { count: stagedTransactions.length })}</h3>
            <button
