...
export const DataInput: React.FC<DataInputProps> = ({ onFileSelect, onFetchFromDb, isLoading }) => {
  ...
  return (
    <div className="bg-card shadow-lg rounded-xl p-8 max-w-3xl mx-auto">
      <h2 className="text-3xl font-bold text-center text-text-primary mb-2">{t('dataInput.title')}</h2>
      <p className="text-center text-text-secondary mb-8">{t('dataInput.subtitle')}</p>
      ...
      <div className="flex border-b border-gray-200 mb-6">
        ...
        >
          <DatabaseIcon /> {t('dataInput.connectTab')}
        </button>
        <button
        ...
        >
          <UploadIcon /> {t('dataInput.importTab')}
        </button>
      </div>
      ...
      {activeTab === 'database' && (
        <div className="text-center h-48 flex flex-col justify-center items-center bg-gray-50 rounded-lg p-4">
            ...
            <h3 className="text-lg font-semibold text-text-primary">{t('dataInput.connectTitle')}</h3>
            <p className="text-sm text-text-secondary mt-1 max-w-md">
                {t('dataInput.connectSubtitle')}
            </p>
            <div className="mt-6">
                 <button
                    ...
                    {isLoading ? (
                        ...
                        {t('dataInput.connecting')}
                        </>
                    ) : t('dataInput.connectButton')}
                 </button>
            </div>
        </div>
      )}
        ...
        <div className="flex flex-col items-center justify-center w-full">
            <label htmlFor="dropzone-file" className={`flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg ${isLoading ? 'cursor-not-allowed bg-gray-100' : 'cursor-pointer bg-gray-50 hover:bg-gray-100'}`}>
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <UploadIcon />
                    <p className="mb-2 text-sm text-text-secondary"><span className="font-semibold">{t('dataInput.uploadTitle')}</span> {t('dataInput.uploadSubtitle')}</p>
                    <p className="text-xs text-text-secondary">{t('dataInput.uploadHint')}</p>
                </div>
