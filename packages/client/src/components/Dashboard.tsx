...
export const Dashboard: React.FC<DashboardProps> = ({ analysis, transactions, aiSummary, isLoading, isAiLoading, accounts, selectedAccount, onAccountChange }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1">
            {t('dashboard.analysisFor')} <span className="font-semibold text-primary">{selectedAccount}</span>
          </p>
        </div>
        {accounts.length > 1 && (
          <div>
            <label htmlFor="account-select" className="sr-only">
              {t('dashboard.filterByAccount')}
            </label>
            <select
              id="account-select"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <SummaryCard title={t('dashboard.totalIncome')} value={formatCurrency(analysis.totalIncome)} icon={<TrendingUpIcon />} color="secondary" />
        <SummaryCard title={t('dashboard.totalExpenses')} value={formatCurrency(Math.abs(analysis.totalExpenses))} icon={<TrendingDownIcon />} color="danger" />
        <SummaryCard title={t('dashboard.netSavings')} value={formatCurrency(analysis.netSavings)} icon={<CheckCircleIcon />} color="primary" />
      </div>
      ...
      {/* AI Summary Section */}
        <div className="bg-white shadow-lg rounded-xl p-6 border border-indigo-100">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-2xl font-bold text-gray-800">{t('dashboard.aiTitle')}</h3>
            </div>
             {isAiLoading ? (
                <div className="flex items-center justify-center p-8 space-x-3">
                    ...
                    <p className="text-text-secondary font-medium">{t('dashboard.aiLoading')}</p>
                </div>
            ) : aiSummary ? (
                <div className="space-y-4 text-gray-600">
                    ...
                    <div className="bg-green-50 border-l-4 border-secondary p-4 rounded-r-lg">
                        <h4 className="font-semibold text-green-800">{t('dashboard.incomeAnalysis')}</h4>
                        <p>{aiSummary.incomeAnalysis}</p>
                    </div>
                    <div className="bg-red-50 border-l-4 border-danger p-4 rounded-r-lg">
                        <h4 className="font-semibold text-red-800">{t('dashboard.expenseAnalysis')}</h4>
                        <p>{aiSummary.expenseAnalysis}</p>
                    </div>
                     <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                        <h4 className="font-semibold text-blue-800 flex items-center gap-2"><LightBulbIcon/> {t('dashboard.actionableInsights')}</h4>
                ...
                <p className="text-text-secondary">{t('dashboard.aiError')}</p>
            )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 bg-card shadow-lg rounded-xl p-6">
            <h3 className="text-xl font-bold mb-4">{t('dashboard.monthlyChartTitle')}</h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={analysis.monthlySummaries} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                ...
                <Bar dataKey="expenses" fill="#ef4444" name={t('common.expense')} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
        </div>
        <div className="lg:col-span-2 bg-card shadow-lg rounded-xl p-6">
          <h3 className="text-xl font-bold mb-4">{t('dashboard.categoryChartTitle')}</h3>
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              <PieWithActiveIndex
      ...
      <div className="bg-card shadow-lg rounded-xl p-6">
        <h3 className="text-xl font-bold mb-4">{t('dashboard.transactionsTitle')}</h3>
